...
type Constants struct {			// This could conceivably change in the engine in future
	...
	GameSeed					int64				`json:"game_seed"`		// Sent to bots but not replay.
}

func NewConstants(players, width, height, turns int, seed int64) *Constants {
...
func NewReplay(names []string, game *Game, turns int, seed int64) *Replay {

	self := new(Replay)

	self.EngineVersion = "Fake Halite Engine"
